import type { Route } from "./+types/api.photoprism.photos";
import { getSessionUser } from "../lib/auth.server";
import { fetchPhotoPrismPhotos } from "../lib/photoprism.server";
...
export async function loader({ request, context }: Route.LoaderArgs) {
	...
	try {
		const { photos, previewToken, hasMore } = await fetchPhotoPrismPhotos(
			PHOTOPRISM_BASE_URL,
			WEBDAV_USERNAME,
			WEBDAV_PASSWORD,
			{ albumUid, offset, count },
		);
		return Response.json({ photos, previewToken, hasMore });
	} catch (err) {
